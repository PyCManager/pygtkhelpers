...
static void     moo_paned_destroy           (GtkWidgetClass      *widget);
...
moo_paned_class_init (MooInternalPanedClass *klass)
{
    ...
    widget_class->destroy = moo_paned_destroy;
                        ...
                        GParamSpec     *pspec)
{
    ...
    {
        case PANED_PROP_ACTIVE_PANE:
            g_value_set_object (value, paned->priv->current_pane);
            break;
                          ...
                          guint     index)
{
    ...
    g_object_notify (G_OBJECT (paned), "active-pane");
    gtk_widget_queue_resize (GTK_WIDGET (paned));
}
