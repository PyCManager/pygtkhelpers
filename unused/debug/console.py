...
class _ReadLine(object):
    ...
    def on_buf_insert(self, buf, itr, text, length):
        if not self.in_raw_input or self.do_insert or not length:
            return
    ...
    def __history(self, dirname):
        text = self._get_line()
        new_text = self.history.get(dirname, text)
...
def _make_window(start_script="from gi.repository import Gtk\n"):
    window = Gtk.Window()
